<x-app-layout>
    <x-slot name="header">
        {{ __('Pelayanan Neonatus: ') }} 
        <a href="/posyandu/{{ $posyandu->id }}" class="text-blue-400 hover:text-blue-700">{{ $posyandu->person->name }}</a>
    </x-slot>

    {{-- komponen anthorpometri --}}
    <div x-data="{ isOpen : true }" class="px-4 py-3 mb-4 bg-white shadow-md dark:bg-gray-800"> 
        <div class="px-4 py-2">
            ...
            <div x-show="isOpen" class="flex justify-end">
                <a href="/posyandu/{{ $posyandu->id }}" class=" bg-green-500 px-4 py-1 mx-4 rounded-md text-white text-sm text-center mt-2 hover:bg-green-700">Kesimpulan Posyandu</a>
                <a href="/posyandu/{{ $posyandu->id }}/anthropometries" class=" bg-green-500 px-4 py-1 rounded-md text-white text-sm text-center mt-2 hover:bg-green-700">Layanan Anthropometri</a>
            </div>
        </div>
